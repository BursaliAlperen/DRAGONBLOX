...
function simDb() {
  const fresh = { session: null, users: {}, withdrawals: {}, catalog: {
    ...
    goals: { dailyBase: 180, dailyPerTier: 45, weekly: 1500 }, withdraw: { minimum: 20 }
  }};
...
function simUser(email, name = email.split('@')[0]) {
  const db = simDb();
  db.users[email] ||= { id: email, email, role: email.includes('admin') ? 'admin' : 'player', profile: { name: name.slice(0, 18), tier: 1, xp: 0, daily: { day: new Date().toISOString().slice(0,10), earned: 0 }, weekly: { week: 'sim', earned: 0 } }, balances: { draco: 500, robux: 100 }, dragons: { ember: { id: 'ember', level: 1, unlockedAt: Date.now() } }, eggs: { cinder: 1 }, withdrawals: [], tx: [], flags: {} };
  db.session = email;
  saveSim(db);
...
function render() {
  ...
  $('#app').innerHTML = `<aside class="rail">
    ...
    ${[['home', 'Dashboard'], ['dragons', 'Dragon Vault'], ['eggs', 'Egg Vault'], ['forge', 'Forge / Sinks'], ['withdraw', 'Robux Desk'], ['history', 'Ledger'], ['admin', 'Admin']].map(([id, label]) => `<button class="nav ${state.tab === id ? 'on' : ''}" onclick="go('${id}')">${label}</button>`).join('')}
...
const views = {
  home() {
    ...
    <h3>Legendary Showcase</h3><div class="grid">${state.catalog.dragons.slice(-4).map(dragonCard).join('')}</div>`;
  },
...
Object.assign(window, { go, login, logout, googleLogin, act, withdraw, adminLoad, review, grant });
boot();
